...
cmd echo $TARGET_HOST; if [ -z "${TARGET_HOST}" ]; then echo "variable \$TARGET_HOST not set"; exit 1; fi \
 ...
 && envsubst '$EXTRA_DIRECTIVES_LOCATION, $EXTRA_DIRECTIVES_SERVER, $DEFAULT_DIRECTIVES_PROXY, $LOG_FORMAT_ESCAPE, $LOG_MAX_BODY_LENGTH,$TARGET_HOST,$LOG_FORMAT,$TARGET_PORT' < /etc/nginx/conf.d/proxy.conf.template > /etc/nginx/conf.d/proxy.conf \
 && nginx -t \
 && nginx
